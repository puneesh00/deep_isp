...
def network(inp_shape, trainable = True):
   ...
   ft224 = Conv2D(32, (3,3), strides = (1,1), padding = 'same', use_bias = True, kernel_initializer = 'he_normal', bias_initializer = 'zeros')(inp_real_imag)
   ...
   ft56dn = resresden(ft56dn, fd//2, gr2, betad, betar, gamma_init, trainable)

   ft28dn = convl(ft56dn, fd, 3, 2, gamma_init, trainable)
   ft28dn = resresden(ft28dn, fd, gr2, betad, betar, gamma_init, trainable)

   ft14dn = convl(ft28dn, fd, 3, 2, gamma_init, trainable)
   #ft56dn = resresden(ft56dn, 128, 32, betad, betar, gamma_init, trainable)

   xrrd = ft14dn
   for m in range(nb):
     xrrd = resresden(xrrd,fd,gr,betad,betar,gamma_init,trainable)

   ft14up = Add()([xrrd,ft14dn])

   ft28up = convl_trans(ft14up, fd, 3, 2, gamma_init, trainable)
   ft28up = resresden(ft28up, fd, gr2, betad, betar, gamma_init, trainable)
   ...
   ft56up = resresden(ft56up, fd//2, gr2, betad, betar, gamma_init, trainable)
   ft56up = Concatenate(axis = -1)([ft56up,ft56dn])
   ...
   ft224up = convl_trans(ft112up, 32, 3, 2, gamma_init, trainable)
   #ft224up = Concatenate(axis = -1)([ft224up,ft224])
   ft224up = resresden(ft224up, 32, 4, betad, betar, gamma_init, trainable)
   ft224up = Concatenate(axis = -1)([ft224up,ft224])

   ft448up = convl_trans(ft224up, 32, 3, 2, gamma_init, trainable)
   ft448up = Add()([ft448,ft448up])

   out = Conv2D(3, (3,3), strides = (1,1), padding = 'same', use_bias = True, kernel_initializer = 'he_normal', bias_initializer = 'zeros')(ft448up)
